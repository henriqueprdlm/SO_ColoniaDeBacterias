// SIMULAÇÃO DE CRESCIMENTO DE COLÔNIAS DE BACTÉRIAS (COM IMPASSE)
...
void* executarColonia(void* colonia_args) {
    ...
    // colônia tenta adquirir os recursos em ordens diferentes dependendo do ID (impasse simulado)
    if (id % 2 == 0) {
        // se o ID for par, tenta obter primeiro o recurso de nutrientes, depois o recurso de espaço
        ...
        // se o ID for ímpar, tenta obter primeiro o recurso de espaço, depois o recurso de nutrientes
        printf("Colônia %d tentando obter recurso de espaço primeiro\n", id);
        recursoEspaco_index = adquirirRecurso(recursoEspaco, NUM_RECURSO_ESPACO, "Espaço", id);                     // trava o mutex do recurso de espaço (obtém o recurso de espaço)
        sleep(1);                                       // simula o tempo de processamento ou espera

        printf("Colônia %d tentando obter recurso de nutrientes\n", id);
        recursoNutrientes_index = adquirirRecurso(recursoNutrientes, NUM_RECURSO_NUTRIENTES, "Nutrientes", id);     // trava o mutex do recurso de nutrientes (obtém o recurso de nutrientes)
    }
